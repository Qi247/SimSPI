...
def find_directed_flows(edges):
    edges = edges.cpu().transpose(1, 0).numpy().tolist()
    # 创建一个邻接字典，用于存储每个节点的出边
    adjacency = {}
    not_start_node = set()
    for in_node, out_node in edges:
        adjacency.setdefault(in_node, []).append(out_node)
        not_start_node.add(out_node)

    # 初始化一个列表，用于存储所有的流
    all_flows = []

    # 定义一个递归函数，从给定的节点开始找出流
    ...
    def dfs(node, path):
        ...
        if node in adjacency:
            ...
            # 如果节点没有出边，将路径添加到all_flows
            all_flows.append(path + [next_node])

    # 从每个节点开始进行DFS
    for start_node in adjacency.keys():
        if start_node in not_start_node:
            continue
...
class GNNNet(nn.Module):
    ...
    class GNNLayer(nn.Module):
        ...
        def cfg_net(self, x, edge_index, edge_attr):
            edge_attr = torch.tensor(edge_attr, dtype=torch.bool)
            edge_index = edge_index[:, edge_attr]
            flows = find_directed_flows(edge_index)
            print("flows:", flows)
            return self.gcns[3](x, edge_index)
        ...
        def forward(self, x, edge_index, edge_attr):
            ...
            for i in range(4):
                ...
                # print(x, edge_index)
                x_t = self.gcns[i](x, edge_index, edge_attr[:, i].contiguous())
                x_t = F.relu(x_t)
                # x_tmp += x_t
                ...
                # break
            x_tmp += torch.mm(
                self.cfg_net(x, edge_index, edge_attr[:, 3].contiguous()),
                self.Ws[3])
...
class Net(nn.Module):
    ...
    def forward(self, Xs):
        ...
        r = self.backnet(code_result_tmp, sim_result_tmp)
...
def eval(model, testloader):
    ...
    return testloss / len(testloader.dataset), [
        true_pos, true_neg, false_pos, false_neg
    ]
...
if __name__ == "__main__":
    ...
    for epoch in range(num_epoch):
        ...
        train_loss, train_matrix_args = train(model, train_dataloader,
                                              optimizer)
        valid_loss, valid_matrix_args = eval(model, valid_dataloader)
        ...
        torch.save(
            ...
            tempPath + "/{:02d}-{:02d}-{:02d}-{:02d}-{:8f}.pkl".format(
                cur.tm_mon, cur.tm_mday, cur.tm_hour, cur.tm_min, valid_acc))
