...
os.system('diff -brN -U 0 -p ' + path + 'a/ ' + path + 'b/ >> ' + path +
          'diff.txt')
...
while 1:
    ...
    if line[:len('diff -brN')] == 'diff -brN':
        ...
        if i < 0:
            ...
            if int(info_a[i + 1:]) > 0:
                hunkInfo.append((filename1, int(info_a[1:i]),
                                 int(info_a[1:i]) + int(info_a[i + 1:]) - 1))
            else:
        ...
        if i < 0:
            ...
            if int(info_b[i + 1:]) > 0:
                hunkInfo.append((filename2, int(info_b[1:i]),
                                 int(info_b[1:i]) + int(info_b[i + 1:]) - 1))
            else:
...
for i in range(0, len(hunkInfo), 2):
    ...
    if hunkInfo[i][2] == 0:
        if hunkInfo[i][0] not in filename:
            ...
            suffix = '\n' * (hunkInfo[i + 1][2] - hunkInfo[i + 1][1] + 1)
            fileContent[idx][hunkInfo[i]
                             [1]] = fileContent[idx][hunkInfo[i][1]] + suffix
        ...
        if hunkInfo[i + 1][0] not in filename:
            ...
            suffix = '\n' * (hunkInfo[i][2] - hunkInfo[i][1] + 1)
            fileContent[idx][hunkInfo[
                i + 1][1]] = fileContent[idx][hunkInfo[i + 1][1]] + suffix
    else:
        gap = (hunkInfo[i][2] - hunkInfo[i][1]) - (hunkInfo[i + 1][2] -
                                                   hunkInfo[i + 1][1])
        #print(gap)
        if gap < 0:
            idx = filename.index(hunkInfo[i][0])
            suffix = '\n' * (-gap)
            fileContent[idx][hunkInfo[i]
                             [2]] = fileContent[idx][hunkInfo[i][2]] + suffix
        elif gap > 0:
            idx = filename.index(hunkInfo[i + 1][0])
            suffix = '\n' * (gap)
            fileContent[idx][hunkInfo[
                i + 1][2]] = fileContent[idx][hunkInfo[i + 1][2]] + suffix
