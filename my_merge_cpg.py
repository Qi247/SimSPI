...
#             g.printAllPaths(dictNode[patchedNodes[i][0]],
#                             dictNode[patchedNodes[j][0]])
#             for p in g.paths:
...
# 如果node节点判断后有return行为或者进入代码块后有return行为就返回true
...
def slice(Anodes, Aedges, Bnodes, Bedges, path, nodeIDstart):
    ...
    for e in Aedges:
        ...
        if edgeType[:3] in ['AST', 'CDG']:
            tmpEdges.append(e)
        elif edgeType[:3] == 'DDG' and len(
                edgeType) > 3 and edgeType[3:] != '<RET>' and any(
                    c.isalpha() for c in edgeType[3:]):
            tmpEdges.append(e)
    Aedges = tmpEdges
    ...
    for Bn in Bnodes:
        for An in Anodes:
            ...
            if AnodeInfo.replace('/a/', '/v/').replace(
                    '/b/', '/v/') == BnodeInfo.replace('/a/', '/v/').replace(
                        '/b/', '/v/') and AnodeInfo and len(AnodeInfo) > 0:
                dictA[AnodeId] = nodeId
                dictB[BnodeId] = nodeId
                CtxNodes.append(
                    (nodeId, AnodeInfo.replace('/a/',
                                               '/v/').replace('/b/',
                                                              '/v/'), 0))
                nodeId -= 1
    ...
    for Bn in Bnodes:
        ...
        if BnodeId not in dictB.keys() and BnodeInfo and len(BnodeInfo) > 0:
            dictB[BnodeId] = nodeId
            PstNodes.append(
                (nodeId, BnodeInfo.replace('/a/', '/v/').replace('/b/',
                                                                 '/v/'), 1))
            nodeId -= 1
    ...
    for An in Anodes:
        ...
        if AnodeId not in dictA.keys() and AnodeInfo and len(AnodeInfo) > 0:
            dictA[AnodeId] = nodeId
            PreNodes.append(
                (nodeId, AnodeInfo.replace('/a/', '/v/').replace('/b/',
                                                                 '/v/'), -1))
            nodeId -= 1
    ...
    for i in range(it_N):
        cnt = len(backNodes3) + len(backEdges3)
        for n2 in backNodes3[:]:
            # nodeID = n[0]
            for e in CtxEdges3 + backEdges3:
                # nodeId2 = e[1]
                if e[1] == n2[0] and e[2][:3] in ['CFG']:
                    for n1 in CtxNodes3[:]:
                        if e[0] == n1[0]:
                            CtxNodes3.remove(n1)
                            ...
                            if e not in backEdges3:
                                backEdges3.append(e)
        if cnt == len(backNodes3) + len(backEdges3):
            break
    ...
    CtxEdges3 = CtxEdges[:]
    CtxNodes3 = CtxNodes[:]
    frwdNodes3 = PreNodes[:] + PstNodes[:]
    frwdEdges3 = PreEdges[:] + PstEdges[:]

    for i in range(it_N):
        cnt = len(frwdNodes3) + len(frwdEdges3)
        for n1 in frwdNodes3[:]:
            for e in CtxEdges3 + frwdEdges3:
                # nodeId2 = e[1]
                if e[0] == n1[0] and e[2][:3] in ['CFG']:
                    for n2 in CtxNodes3[:]:
                        if e[1] == n2[0]:
                            CtxNodes3.remove(n2)
                            frwdNodes3.append(n2)
                            if e in CtxEdges3[:]:
                                CtxEdges3.remove(e)
                            if e not in frwdEdges3:
                                frwdEdges3.append(e)
        if cnt == len(frwdNodes3) + len(frwdEdges3):
            break

    frwdNodes = list(set(frwdNodes1 + frwdNodes2 + frwdNodes3))
    frwdEdges = list(set(frwdEdges1 + frwdEdges2 + frwdEdges3))
                ...
                if lineNum_list[i].isnumeric():
                    if str(int(lineNum_list[i])+j) in lineNum_list and \
                     lineNum_code[str(int(lineNum_list[i])+j)] in lineNum_code[lineNum_list[i]]:
                        oldNum_newNum[str(int(lineNum_list[i]) +
                                          j)] = lineNum_list[i]
                        flag_cnt += 1
                else:
                    if lineNum_list[i][0]+str(int(lineNum_list[i][1:])+j) in lineNum_list and \
                     lineNum_code[lineNum_list[i][0]+str(int(lineNum_list[i][1:])+j)] in lineNum_code[lineNum_list[i]]:
                        oldNum_newNum[lineNum_list[i][0] +
                                      str(int(lineNum_list[i]) +
                                          j)] = lineNum_list[i]
                        flag_cnt += 1
        i += (flag_cnt + 1)

    for line_num in list(set(oldNum_newNum.values())):
        slimNodes.append((lineNum_nodeID[line_num], lineNum_label[line_num],
                          line_num, lineNum_code[line_num]))
    ...
    for n in slimNodes:
        if n[2].isnumeric():
            ...
            for nodeID2 in node_lineNum.keys():
                if (n[0], nodeID2) in simple_edge or (nodeID2,
                                                      n[0]) in simple_edge:
                    tmp_dist = min(
                        [tmp_dist,
                         abs(int(n[2]) - node_lineNum[nodeID2])])
            reservedNodes.append(
                (n[0], n[1], node_type[n[0]], tmp_dist, n[2], n[3]))
        else:
...
def generateLog(path):
    ...
    for f in ABFunc:
        ...
        if len(Anodes) > 300 and len(Bnodes) > 300:
            continue

        sliceEdges, sliceNodes, nodeID = slice(Anodes, Aedges, Bnodes, Bedges,
                                               path, nodeIDstart)
        if len(sliceEdges) > 0 and len(sliceNodes) > 0:
            allEdges_list.append(sliceEdges)
            allNodes_list.append(sliceNodes)
    ...
    for allEdges in allEdges_list:
        ...
        for e in allEdges:
            if e[-1] == 0:
                ...
                Bedges_tmp.append(e)
        AEdges_list.append(Aedges_tmp)
        BEdges_list.append(Bedges_tmp)
    for allNodes in allNodes_list:
        ANodes_tmp = []
        BNodes_tmp = []
        for n in allNodes:
            if n[1] == 0:
                ...
                BNodes_tmp.append(n)
        ANodes_list.append(ANodes_tmp)
        BNodes_list.append(BNodes_tmp)
        print(ANodes_list)

    if len(allEdges_list) > 0 or len(allNodes_list) > 0:

        out_path = path.replace('ab_file', 'testdata1')
        print(out_path)
        if not os.path.exists(out_path):
            os.system('mkdir ' + out_path)
        for i in range(len(allEdges_list[:5])):
            f = open(out_path + '/out_slim_ninf_noast_n1_w_{:d}.log'.format(i),
                     'w+')
            f.write('\n'.join(map(str, allEdges_list[i])))
            f.write("\n===========================\n")
            f.write('\n'.join(map(str, allNodes_list[i])))
            f.write("\n---------------------------\n")
            f.write('\n'.join(map(str, AEdges_list[i])))
            f.write("\n===========================\n")
            f.write('\n'.join(map(str, ANodes_list[i])))
            f.write("\n---------------------------\n")
            f.write('\n'.join(map(str, BEdges_list[i])))
            f.write("\n===========================\n")
            f.write('\n'.join(map(str, BNodes_list[i])))
            f.write("\n")
            f.close()
        return 1
    return 0
...
if not os.path.exists('./testdata1/'):
    os.mkdir('./testdata1/')
...
commits = sorted(files)

c = 0
i = 0
for cmt in commits[c:1000]:
    print("starting: ", c, cmt)
    c += 1
    ...
    if os.path.exists(cmt + '/outA') and os.path.exists(cmt + '/outB'):
        # if os.path.exists(
        #         cmt.replace("ab_file", "testdata1") +
