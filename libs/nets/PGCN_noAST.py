...
class PGCN(torch.nn.Module):  # PGCN
    ...
    def __init__(self, num_node_features):
        ...
        self.mlp = Sequential(Linear(num_node_features * 2, 8), ReLU(),
                              Linear(8, 2))
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        ...
        # xA = self.conv1A(x, edge_index[0]).relu()
        # xB = self.conv1B(x, edge_index[1]).relu()
        # xC = self.conv1C(x, edge_index[2]).relu()
...
class PGCN_g(torch.nn.Module):  # PGCN_global
    ...
    def __init__(self, num_node_features):
        ...
        self.mlp = Sequential(Linear(num_node_features * 14, 32), ReLU(),
                              Linear(32, 8), ReLU(), Linear(8, 2))
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        ...
        pool, pool_index, _, pool_batch, _, _ = self.pool(gcn,
                                                          edge_index,
                                                          batch=batch)
        readout = torch.cat([
            global_mean_pool(pool, pool_batch),
            global_max_pool(pool, pool_batch)
        ],
                            dim=1)  # [batch_size, hidden_channels]
...
class PGCN_h(torch.nn.Module):  # PGCN_hierarchical
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        ...
        # x = (xA + xB + xC + xD) / 4.0 # average
        gcn1 = torch.cat((xA, xB, xC, xD), dim=1)  # concat
        pool1, edge_index1, _, batch1, _, _ = self.pool1(gcn1,
                                                         edge_index,
                                                         batch=batch)
        global_pool1 = torch.cat(
            [global_mean_pool(pool1, batch1),
             global_max_pool(pool1, batch1)],
            dim=1)

        # Layer 2
        gcn2 = self.conv2(pool1, edge_index1).relu()
        pool2, edge_index2, _, batch2, _, _ = self.pool2(gcn2,
                                                         edge_index1,
                                                         batch=batch1)
        global_pool2 = torch.cat(
            [global_mean_pool(pool2, batch2),
             global_max_pool(pool2, batch2)],
            dim=1)

        # Layer 3
        gcn3 = self.conv3(pool2, edge_index2)
        pool3, edge_index3, _, batch3, _, _ = self.pool3(gcn3,
                                                         edge_index2,
                                                         batch=batch2)
        global_pool3 = torch.cat(
            [global_mean_pool(pool3, batch3),
             global_max_pool(pool3, batch3)],
            dim=1)

        # 2. Readout layer
        readout = torch.cat([global_pool1, global_pool2, global_pool3],
                            dim=1)  # [batch_size, hidden_channels]
...
def PGCNTrain(model, trainloader, optimizer, criterion):
    ...
    return model, lossTrain, [tp, tn, fp, fn]
