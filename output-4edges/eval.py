...
def GetDataset(path=None):
    ...
    train_dataset = []
    valid_dataset = []
    train_dataset.extend(pos_dataset[:int(len(pos_dataset) * 0.8)])
    train_dataset.extend(neg_dataset[:int(len(neg_dataset) * 0.8)])
    valid_dataset.extend(pos_dataset[int(len(pos_dataset) * 0.8):])
    valid_dataset.extend(neg_dataset[int(len(neg_dataset) * 0.8):])
