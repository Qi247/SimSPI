...
forbidden_l = [328, 673, 791, 796, 801, 802, 810, 1059]
c = forbidden_l[-1] + 1
i = 0
for cmt in commits[c:2000]:
    if c in forbidden_l:
        c += 1
        continue
    print(c, cmt)
    c += 1
    if os.path.isfile(cmt + '/cpg_a.txt') and os.path.isfile(cmt +
                                                             '/cpg_b.txt'):
        # if os.path.exists(
        #         cmt.replace("ab_file", "testdata") +
        #         "/out_slim_ninf_noast_n1_w.log"):
        #     continue

        time0 = time.time()
        os.system('cd ./joern; ./joern-parse .' + cmt +
                  '/a; ./joern-export --repr cpg14 --out .' + cmt + '/outA')
        time1 = time.time()
        if time1 - time0 > 120:
            forbidden_l.append(c - 1)
            print(forbidden_l)
            continue
        os.system('cd ./joern; ./joern-parse .' + cmt +
                  '/b; ./joern-export --repr cpg14 --out .' + cmt + '/outB')
