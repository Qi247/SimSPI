...
ggnn_args = {"out_channels": 20, "num_layers": 2, "aggr": "add", "bias": True}
...
class GNNNet(nn.Module):
    ...
    class GNNLayer(nn.Module):
        ...
        def __init__(self, gated_graph_conv_args: Dict[str, int]):
            ...
            self.gcns = [
                GCNConv(gated_graph_conv_args['out_channels'],
...
class Net(nn.Module):
    ...
    def backnet(self, code_result_tmp):
        code_result = self.code_linear1(
            code_result_tmp)  # N x K*2 x 128 -> N x K*2 x 13
    ...
    def forward(self, Xs):
        ...
        code_result_tmp = torch.empty(0,
                                      ...
                                      device=device)
        for X in Xs:
            g_matrix = self.gnnnet(X)
            code_result_tmp = torch.cat((code_result_tmp, g_matrix), 0)
        r = self.backnet(code_result_tmp)
