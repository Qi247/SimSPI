...
def main():
    ...
    for root, ds, fs in os.walk(mdatPath):
        for file in fs:
            ...
            #     print('=====================================================')
            #     continue
            print('[INFO] <main> Process the graph numpy file: [' + str(cnt) +
                  '] ' + filename + RunTime())
            # =====================================================
            nodes, edges, nodes0, edges0, nodes1, edges1, label = ReadFile(
                filename)
            ...
            if _TWINS_:
                ...
                np.savez(savename2,
                         ...
                         label=label)
                print(
                    '[INFO] <main> save the graph information (twins) into numpy file: ['
                    + str(cnt) + '] ' + savename2 + RunTime())
                print('=====================================================')
            # =====================================================
...
def ProcEdges(edgesData):
    ...
    if 0 == len(edgesData):
        ...
        return {}, np.array([[0], [0]]), np.zeros(
            (1, 6))  # one edge, attr: [0 0 0 0 0 0]
    ...
    print('[INFO] <ProcEdges> Find', len(nodeDict), 'nodes connected with',
          len(edgesData), 'edges.' + RunTime())
    ...
    print('[INFO] <ProcEdges> Get', len(edgeIndex), '*', len(edgeIndex[0]),
          'edge index array.' + RunTime())
    ...
    #     edge_type = typeDict[edgesData[i][2]]
    #     edgeIndex_list[edge_type][0].append(nodesoutIndex[i])
    #     edgeIndex_list[edge_type][1].append(nodesinIndex[i])
    ...
    print('[INFO] <ProcEdges> Get', len(edgeAttr), '*', len(edgeAttr[0]),
          'edge attribute array.' + RunTime())
...
def GetNodeEmbedding(nodeData):
    ...
    for i in range(numTokens):
        ...
        if (2 == tokenTypes[i]):
            for item in [
                    'alloc', 'free', 'mem', 'sizeof', 'new', 'delete', 'open',
                    'close', 'create', 'release', 'copy', 'remove', 'clear',
                    'dequene', 'enquene', 'detach', 'attach'
            ]:
            ...
            for item in ['null', 'nil', 'none']:
                if item in tokens[i].lower():
                    count_null = 1
            for item in [
                    'put', 'get', 'init', 'register', 'down', 'up', 'disable',
                    'enable', 'sub', 'add', 'dec', 'inc', 'set', 'stop',
                    'start', 'suspend', 'resume', 'connect', 'map', 'prepare'
            ]:
                if item in tokens[i].lower():
                    count_API = 1
    ...
    for i in range(numTokens):
        if (1 == tokenTypes[i]) and (tokens[i] in ['if', 'switch']):
            ...
            count_loop = 1
        elif (1 == tokenTypes[i]) and (tokens[i] in [
                'return', 'break', 'continue', 'goto', 'throw', 'assert'
        ]):
            count_jump = 1
...
def ProcNodes(nodesData, nodeDict):
    ...
    # print(nodeAttr)
    print('[INFO] <ProcNodes> Get', len(nodeAttr), '*', len(nodeAttr[0]),
          'node attribute array.' + RunTime())
