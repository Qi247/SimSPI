...
def main():
    ...
    for root, ds, fs in os.walk(dataPath):
        for file in fs:
            ...
            subfolder = '/positives/' if ('positives'
                                          in root) else '/negatives/'
...
def ParseEdge(filename, line):
    ...
    if 0 == len(contents):
        if _ERROR_:
            print('[ERROR] <ParseEdge> Edge does not match the format, para:',
                  filename, line)
        return []
    ...
    if segs[2].startswith('DDG'):
        segs[2] = 'DDG'
    if not segs[2] in ['CDG', 'DDG', 'CFG', 'AST']:
        if _ERROR_:
            print('[ERROR] <ParseEdge> Edgetype Error, para:', filename, line)
        return []
...
def ParseNode(filename, line):
    ...
    if 0 == len(contents):
        if _ERROR_:
            print('[ERROR] <ParseNode> Node does not match the format, para:',
                  filename, line)
        return []
    content = contents[0]
    ...
    ret = np.array([segs[0], segs[1], segs[2], segs[3], segs[4], [0], [attr]],
                   dtype=object)
    ...
    for line in lines:
        ...
        if line.startswith('---'):
            ...
            if 0 == signGraph:
                ...
                if _ERROR_:
                    print(
                        '[ERROR] <ReadFile> Find abnormal additional graphs, para:',
                        filename)
        elif (0 == signEdge):
            # Node:
            ...
            # Error:
            if _ERROR_:
                print('[ERROR] <ReadFile> Neither an edge or a node, para:',
                      filename, line)

    print(
        f'[INFO] <ReadFile> Read PatchCPG (#node: {len(nodesData)}, #edge: {len(edgesData)}), ',
        end='')
    print(f'PreCPG (#node: {len(nodesData0)}, #edge: {len(edgesData0)}), ',
          end='')
    print(f'PostCPG (#node: {len(nodesData1)}, #edge: {len(edgesData1)}).' +
          RunTime())
    if _DEBUG_:
        print(nodesData)
        print(edgesData)
...
def ProcNodes(nodesData, gtype='CPG'):
    ...
    print(
        f'[INFO] <ProcNodes> Tokenize code for {len(nodesData)} nodes in {gtype}.'
        + RunTime())
